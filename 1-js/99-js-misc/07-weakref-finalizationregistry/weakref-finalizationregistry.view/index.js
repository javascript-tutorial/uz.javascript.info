...
// Renderlar.
// Thumbnail ko'rinishlarini render qilish.
images.forEach((img) => {
...
// Layout tanlash ro'yxatini render qilish.
...
const handleStateChange = (state) => {
  if (state.loading) {
    ...
    spinnerTextEl.innerText = "Yuklanyapti...";
...
const drawCollage = (images) => {
  ...
  /**
   * Joriy layoutga asoslanib canvas o'lchamlarini hisoblash.
   * */
  ...
  for (let i = 0; i < images.length; i++) {
    /**
     * Kollajning joriy qatorini olish.
     * */
...
/**
 * Qaytadan boshlash uchun barcha belgilangan ma'lumotlarni tozalash.
 * */
const startOver = () => {
...
const downloadCollage = () => {
  const date = new Date();
  const fileName = `Kollaj-${date.getDay()}-${date.getMonth()}-${date.getFullYear()}.png`;
